# =============================================================================
# sim_power.R  --  Monte Carlo inner loop for the power analysis (Figure 1)
#
# Sourced by main_drdid_power.R with `dgp`, `n`, `delta`, `job` in scope. For
# each of `nrep` replications it generates data via dgps_did(delta, ...), fits
# the GPS/OR by cross-validated local polynomials, computes the stationary and
# non-stationary DR DiD estimators and the Hausman-type test statistic, and
# stores them in the 60-column matrix `mc`. Writes per-delta RData and summary
# CSVs to results/.
...
# =============================================================================
# ---------------------------------------------------------------------------
# Monte Carlo simulation for doubly robust DID (power analysis)
# ---------------------------------------------------------------------------

temp_data <- paste0(address, "/results/", "temp_power.dgp-", dgp,
                   ".delta-", delta, ".n-", n, ".job-", job, ".RData")
...
outData <- paste0(address, "/results/", "rlst_power.dgp-",dgp,".delta-",delta,".n-",n,".job-", job,".RData")
save.image(outData)

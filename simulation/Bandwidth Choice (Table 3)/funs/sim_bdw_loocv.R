# =============================================================================
# sim_bdw_loocv.R  --  Monte Carlo inner loop for the bandwidth study (Table 3)
#
# Sourced by main_bdw_loocv.R with `dgp`, `n`, `job`, `delta` in scope. For
# each of `nrep` replications it generates data via dgps_did(), selects the
# local-polynomial bandwidths by LOOCV, fits the GPS/OR, computes the DR DiD
# estimators and the Hausman-type test, and stores them in the 60-column matrix
# `mc`. Writes per-job RData and summary CSVs to results/loocv/.
...
# =============================================================================
# ---------------------------------------------------------------------------
# Monte Carlo simulation for doubly robust DID (LOOCV bandwidths)
# ---------------------------------------------------------------------------

temp_data <- paste0(address, "/results/loocv/", "temp_loocv.dgp-", dgp,
                   ".n-", n, ".job-", job, ".RData")
...
for (imc in 1:nrep) {
  ...
  mc[imc, ] <- matrix(c(
    ...
    # Two-way fixed effect estimators  # Placeholders
    # baseline
    att_twfe1,  #52
...
out1 <- paste0("mc.summary-dr-stb",".dgp-",dgp,".delta-",delta,".n-",n,".job-", job,".csv")
out1 <- paste0(address,"/results/loocv/", out1, sep="")
...
out2 <- paste0("mc.summary-dr-unstb",".dgp-",dgp,".delta-",delta,".n-",n,".job-", job,".csv")
out2 <- paste0(address,"/results/loocv/", out2, sep="")
...
out3 <- paste0("mc.summary-dr-test",".dgp-",dgp,".delta-",delta,".n-",n,".job-", job,".csv")
out3 <- paste0(address,"/results/loocv/", out3, sep="")
...
outData <- paste0(address, "/results/loocv/", "rlst_loocv.dgp-",dgp,".n-",n,".job-", job,".RData")
save.image(outData)
